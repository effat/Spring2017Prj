...
#remove whitespaces, list all keywords
privacy_keys=[]
for i in range(0,len(df)):
    word=df.iloc[i,0]
    word=''.join(word.split())
    privacy_keys.append(word)
...
# threshold for number of keywords
threshold_k=20#len(final_keys)
...
try:
    ...
    appendFile = open('privacyArticles_k_20.csv','w') #privacyArticles_4_25_Top20.csv
    ...
    while result is not None:
        ...
        if sum>=threshold_k:
            classLabel=1 #if threshold_k keywords are present, it is a positive example
